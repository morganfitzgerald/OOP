...
#Custom imports
from src.utils import subject_file_mapping, extract_data, simulate_ecg_sig, epoch_cycles
from src.template_matching import match_ecg_template
from src.feature_extraction import nk_peaks, find_peak_params, calc_shape_params, calc_bounds
from src.analysis import find_extremum, calc_fwhm, find_peak_onset_offset, calc_r_squared, gaussian_function, gaussian_std
...
# ## Template matching
SUB_NUM = '1' 
dir_path = '/Users/morganfitzgerald/Projects/ecg_tool_validation/data/AutoAge_data/raw'
files_dat_dict, files_hea_dict = subject_file_mapping(dir_path)
...
#SELECT THE SIGNAL
template_ecg = simulate_ecg_sig(duration=5, sampling_rate=fs, heart_rate=80, amplitude_factor=7, normalize=False)
...
#PARAMETERIZATION: 
print(f"starting {SUB_NUM} parameterization")
...
for cycle in epochs_df['cycle'].unique():  # np.arange(0, 20):  #for all limited cycles
    ...
    le_ind_r, ri_ind_r = find_peak_onset_offset(sig, r_ind, peak_height=r_height)
    ...
    for comp, params in component_inds.items():
        onset, offset = find_peak_onset_offset(sig, peak_index=params[0], peak_height=params[1])
...
ecg_output.to_csv(f'../docs/saved_files/timedomain_results/{SUB_NUM}_ecg_cycle_data.csv', index=False)
