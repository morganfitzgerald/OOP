...
def find_extremum(xs, sig, start_idx, end_idx, mode):
    """
    ...
    if mode == 'min':
        idx_relative = np.argmin(sig[start_idx:end_idx])
    else:
        idx_relative = np.argmax(sig[start_idx:end_idx])
    
    idx_absolute = idx_relative + start_idx
    value = sig[idx_absolute]
