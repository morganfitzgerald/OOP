...
# Define the directory where your newly created files are located
directory = "/Users/morganfitzgerald/Projects/ecg_param/sims/timeseries"
# Directory where results are saved
results_dir = "/Users/morganfitzgerald/Projects/ecg_param/sims/timedomain_results/"


# In[3]:
# Loop through each file in the directory
for file_name in os.listdir(directory):
    # Construct the expected path for the results file
    expected_results_file = os.path.join(results_dir, f"{file_name}_ecg_output.csv")

    # Check if the results file already exists
    if os.path.exists(expected_results_file):
        print(f"Results for subject {file_name} already exist. Skipping.")
        continue  # Skip this subject and move to the next

    # Check if the file is a numpy file
    if file_name.endswith(".npy"):
        # Load the noisy signal from the numpy file
        ecg = np.load(os.path.join(directory, file_name))

        # Print the file name and any other information as needed
        ...
        b, a = butter(order, cutoff_frequency, btype="high", analog=False, fs=1000)
        ...
        epochs_nk_df, _ = epoch_signals(
            p_peaks_nk, ecg_clean_nk, FS, file_name, PLOT=False, SAVE=False
        )
        ...
        epochs_df, result_r_latencies = epoch_signals(
            p_peaks_nk, ecg_notch, FS, file_name, PLOT=False, SAVE=False
        )
        ...
        ecg_output = pd.DataFrame(
            {
                ...
                "sharpness_deriv_p": np.zeros(len(epochs_df["cycle"].unique())),
                "sharpness_diff_p": np.zeros(len(epochs_df["cycle"].unique())),
                ...
                "sharpness_deriv_q": np.zeros(len(epochs_df["cycle"].unique())),
                ...
                "sharpness_deriv_r": np.zeros(len(epochs_df["cycle"].unique())),
                "sharpness_diff_r": np.zeros(len(epochs_df["cycle"].unique())),
                ...
                "sharpness_deriv_s": np.zeros(len(epochs_df["cycle"].unique())),
                ...
                "sharpness_deriv_t": np.zeros(len(epochs_df["cycle"].unique())),
                "sharpness_diff_t": np.zeros(len(epochs_df["cycle"].unique())),
                ...
                "NN50": np.zeros(len(epochs_df["cycle"].unique())),
            }
        )
        ...
        for cycle in np.arange(0, len((epochs_df["cycle"].unique()))):
            ...
            for comp, params in component_inds.items():
                ...
                ecg_output.loc[cycle, f"{comp}_on"] = (
                    xs[onset] if onset is not None else np.nan
                )
                ecg_output.loc[cycle, f"{comp}_off"] = (
            ...
            for ii, peak in enumerate(peak_params):
                ...
                half_mag = int(
                    np.abs(peak[1] / 2)
                )  # peak[1] represents the height of the peak
                half_mag_cropped_xval = np.argmin(np.abs(sig - half_mag))
                # half_mag_real_xval = xs[half_mag_cropped_xval]

                # left_index  = max(0, peak_index - half_mag_cropped_xval)
                # right_index = min(len(sig) -1, peak_index + half_mag_cropped_xval)
                ...
                )  # peak[0] refers to the center (or index) of a peak in a signal, often derived from the Gaussian parameters obtained from curve fitting.
                sharpness_diff = np.abs(sharpness_diff)

                # compute sharpness (using deriv)
                cropped_peak_sig = sig[
                    max(0, peak_index - half_mag_cropped_xval) : min(
                        len(sig) - 1, peak_index + half_mag_cropped_xval + 1
                    )
                ]
                sharpness_deriv = np.mean(np.abs(np.diff(cropped_peak_sig)))
            ...
            ecg_output.loc[cycle, "p_duration"] = (
                ecg_output.loc[cycle, "p_off"] - ecg_output.loc[cycle, "p_on"]
            )
            ecg_output.loc[cycle, "pr_interval"] = (
                ecg_output.loc[cycle, "q_on"] - ecg_output.loc[cycle, "p_on"]
            )
            ecg_output.loc[cycle, "pr_segment"] = (
                ecg_output.loc[cycle, "q_on"] - ecg_output.loc[cycle, "p_off"]
            )
            ecg_output.loc[cycle, "qrs_duration"] = (
                ecg_output.loc[cycle, "s_off"] - ecg_output.loc[cycle, "q_on"]
            )
            ecg_output.loc[cycle, "st_segment"] = (
                ecg_output.loc[cycle, "t_off"] - ecg_output.loc[cycle, "s_off"]
            )
            ecg_output.loc[cycle, "qt_interval"] = (
                ecg_output.loc[cycle, "t_off"] - ecg_output.loc[cycle, "q_on"]
            )
            ...
            ecg_output.loc[cycle, 25:55] = shape_params_flat
        ...
        # Save output in new file
        ecg_output.to_csv(
            f"/Users/morganfitzgerald/Projects/ecg_param/sims/docs/timedomain_results/{file_name}_ecg_output.csv"
        )


#     except Exception as e:
#         print(f"An error occurred while processing subject {file_name}: {e}")
# else:
#     print(f"Header file for subject {file_name} not found.")
