import numpy as np
from src.feature import extract_peak_indices 
from src.analysis import calc_sharpness_deriv
...
def calc_shape_params(xs, sig, peak_params):
    ...
    for ii, peak in enumerate(peak_params):
        ...
        start_index, peak_index, end_index = extract_peak_indices(xs, sig, peak)
        ...
        # Store the start, peak, and end indices
        peak_bounds[ii] = [start_index, peak_index, end_index]

    # Return the shape parameters and peak bounds
    return shape_params, peak_bounds
